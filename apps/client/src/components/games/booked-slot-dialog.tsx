...
const BookedSlotDialog = ({
    ...
    const waitForSpecificSlotMutation = useWaitForSpecificSlot(
        gameId ?? '',
        slot?.id ?? ''
    );
    ...
    const handleWaitForSlot = () => {
        ...
        waitForSpecificSlotMutation.mutate(payload, {
            onSuccess: () => {
                setOpenState(false);
            },
        });
            ...
            <DialogContent className="flex max-h-[min(600px,80vh)] flex-col gap-0 p-0 sm:max-w-md">
                ...
                <DialogFooter className="flex-row items-center justify-end border-t px-6 py-4 gap-2">
                    ...
                    {shouldShowWaitButton && (
                        <Button
                            disabled={waitForSpecificSlotMutation.isPending}
                            onClick={handleWaitForSlot}
                        >
                            {waitForSpecificSlotMutation.isPending ? (
                                <span className="flex items-center">
                                    <Spinner className="mr-2 h-4 w-4 animate-spin" />
                                    Waiting...
                                </span>
